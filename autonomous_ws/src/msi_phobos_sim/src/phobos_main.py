...
def imu_bearing_callback(data):
	...
	rospy.loginfo("IMU@UDP hit! %s", data.data[0] * 180/np.pi)
	rover_bearing = data.data[0]
	imu_void = False
...
def calibrate_direction(dest):
	...
	rospy.loginfo("Turning %s degrees", diff)
	...
	while abs(fine_diff) > ang_epsilon:
		# Fine tuning
...
def init_topics():
	...
	rospy.Subscriber("IMU", Float64MultiArray, imu_bearing_callback)
	rospy.Subscriber("/drive/controller_feedback", Bool, drive_cc_callback)
	# Subscribe to joystick message! @agrim9
	ang_pub = rospy.Publisher("/ang_msg", Float64, queue_size=10) # fwd, bck, acw, cw, hlt
	drive_pub = rospy.Publisher("/drive_msg", Float64, queue_size=10) # fwd, bck, acw, cw, hlt
...
if __name__ == '__main__':
	...
	while (get_distance(source, destination) * 1000.) > destination_dist_thresh:
		...
		distance_mag = min(get_distance(curr_pos - gps_offset, destination), abs(checkpoint_leg)) * 1000
		drive_cc_status = False
